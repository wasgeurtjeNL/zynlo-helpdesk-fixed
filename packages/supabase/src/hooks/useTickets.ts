  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      for (let i = 0; i < ticketIds.length; i += batchSize) {
        ...
        const { error } = await supabase.from('tickets').update(updateData).in('id', batch);
    ...
    mutationFn: async (params: {
      ...
      if (assigneeId) {
        ...
        for (const ticketId of ticketIds) {
          ...
          const { error } = await supabase.from('tickets').update(ticketUpdate).eq('id', ticketId);
        ...
        const { error } = await supabase.from('tickets').update(updates).in('id', ticketIds);
