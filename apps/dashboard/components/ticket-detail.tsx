      ...
      <div className="px-4 lg:px-6 py-4 space-y-4">
        {messages.map((message, index) => {
          ...
          return (
            ...
            >
              <div
                className={cn('flex gap-3', message.sender_type === 'agent' && 'flex-row-reverse')}
              >
...
function ReplyPanel({ ticket, onSendMessage, userSignature, visible, onScroll }: ReplyPanelProps) {
  ...
  const { data: savedReplies = [] } = useSavedReplies(user?.id || '');
  ...
  const handleAISuggestion = async () => {
    ...
    try {
      ...
      const customerMessages =
        ticket.messages?.filter((m: Message) => m.sender_type === 'customer') || [];
      ...
      if (!response.ok) {
        if (response.status === 401) {
          showToast(
            'error',
            `Authenticatie fout: ${data.details || data.error}. Debug: ${JSON.stringify(data.debug)}`
          );
        } else if (response.status === 429) {
          showToast(
            'error',
            `AI limiet bereikt: ${data.usage?.requests_used}/${data.usage?.requests_limit} verzoeken gebruikt`
          );
        } else if (response.status === 500 && data.error === 'OpenAI API key not configured') {
          showToast(
            'error',
            'OpenAI API key is niet geconfigureerd. Voeg OPENAI_API_KEY toe aan je .env.local bestand.'
          );
        } else {
      ...
      if (data.suggestions && data.suggestions.length > 0) {
        ...
        (window as any).__aiSuggestions = data.suggestions(window as any).__currentSuggestionIndex =
          0;
      }
          ...
          {!isExpanded && (
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  {(newMessage || draft) && (
                    <span className="text-xs text-orange-600 font-medium">Concept opgeslagen</span>
                  )}
                ...
                <div className="flex items-center justify-between p-3 border-t border-gray-200 bg-gray-50">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      title={
                        isInternalNote
                          ? 'Schakel naar klantbericht'
                          : 'Schakel naar interne notitie'
                      }
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        showSuggestions
                          ...
                          : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100',
                        (isLoadingAI || (aiUsageStatus && !aiUsageStatus.allowed)) &&
                          'opacity-50 cursor-not-allowed'
                      )}
                      title={
                        aiUsageStatus && !aiUsageStatus.allowed
...
export function TicketDetail({ ticketNumber }: TicketDetailProps) {
  ...
  useEffect(() => {
    if (prevAssigneeId.current !== effectiveTicket?.assignee_id) {
      console.log(
        'Effective ticket assignee_id changed:',
        prevAssigneeId.current,
        '->',
  ...
  const handleSendMessage = async (content: string, isInternal: boolean) => {
    ...
    try {
      if (isInternal) {
        ...
        console.log('🔥 DEBUG: Email response headers:', Object.fromEntries(emailResponse.headers));
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          console.log(
            'Direct DB check - ticket assignee_id:',
            checkTicket?.assignee_id,
            'error:',
            checkError
          );
            ...
            <div className="flex items-center gap-3 text-xs text-gray-500">
              {effectiveTicket.assignee_id && (
                <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full">
                  Toegewezen aan{' '}
                  {effectiveTicket.assignee?.full_name || effectiveTicket.assignee?.email}
                </span>
              )}
            </div>
          </div>
